...
namespace EvChargerUI.Services.DspControl.Chaevi
{
    public class DspControl
    {
        ...
        private void ReadRegister(int channel)
        {
            ...
            try
            {
                if (!client.SendFc3(_channelIds[channel], 400, 20))
                {
                    IsConnected = false;
                    return;
                }
                ...
                if ((int)idnum[0] == (int)this._channelIds[channel] && idnum[1] == 3)
                {
            ...
            {
                Console.WriteLine("Chaevi DspControl ReadRegister Exception: " + ex.Message);
                Console.WriteLine("Chaevi Exception type: " + ex.GetType().Name);
                Console.WriteLine("Chaevi Exception message: " + ex.Message);
        ...
        private void ReadRegister(int channel)
        {
            _modbus.SendFc3(_channelIds[channel], 400, 20);
        ...
        private void WriteRegister(TxData data)
        {
            try
            {
                ...
                if (data.Stop) _logger.Debug("[SEND] SEND STOP!!!");
                if (!_modbus.SendFc16(_channelIds[data.ChannelNo], 200, 9, rawData))
                {
                    IsConnected = false;
                    return;
                }
            ...
            {
                Console.WriteLine("Chaevi DspControl WriteRegister Exception: " + ex.Message);
                IsConnected = false;
            }
        }
